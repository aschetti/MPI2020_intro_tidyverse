...
rmANOVA_att <- aov_ez(
  ...
  att_filter_select_recode_long_sep_onego, # data frame
...
```{r install_load_bootES, eval = FALSE}
...
att_bootES <-
  ...
  mutate( # compute mean differences
...
att_HedgesG <-
  att_bootES %>%
  group_by(diff_conds) %>%
  nest() %>% # create lists with difference scores
  mutate(
    bootstrap = map(
      data, # name of the column with lists
      ~ bootES(
        ., # data
...
unnest(att_HedgesG, tidy)


# magrittr::extract2(att_HedgesG, "bootstrap")
# 
# magrittr::extract2(att_HedgesG, "tidy")
# 
# 
# att_HedgesG <-
#   att_bootES %>%
#   split(.$diff_conds) %>% # split difference scores in separate lists
...
The result is a data frame containing `r length(unique(att_bootES$diff_conds))` lists (i.e., the number of ```diff_conds```), each containing the results of the bootstrapping procedure. As an example, let's look at the list containing the bootstrapped Hegdes's _g_ for the difference ratings of beer after seeing negative vs. neutral advertisement:

```{r bootES_purrr_output}
att_HedgesG$beer_negativeVSbeer_neutral
```
