...
1. participant #98 contains missing values (**NA**)
2. data of participant #99 are implausible, perhaps due to technical problems (e.g., the rating scale was out of the -100/100 range)  
...
you can see that the ratings of participant #99 are a bit strange... perhaps there was a technical problem, i.e., the rating scale was out of the -100/100 range.   
...
How to verify that the filtering worked? You could click on  ```att``` in the *Environment* window and look for participant #99, but it can be tedious with big data sets. An easy and flexible way is to check if the filtered data set still contains participant #99:
...
Let's inspect the variable ```gender```:

```{r recode_look}

att_filter_select$gender
...
unique(att_filter_select$gender)
...
class(att_filter_select$gender)

```

Class ```numeric``` has numeric values (i.e., double precision floating point numbers).

There are many variable types in **R**. An interesting one is ```factor```, i.e., variables which are assigned specific values (you can think of them as **categorical** variables). In our case, it would make sense to consider ```gender``` as a ```factor``` with two levels, *male* and *female*. Let's transform it:
...
att_filter_select_recode <-
  att_filter_select %>%
  mutate( # create new variables in tibble
    sex = recode( # the new variable 'sex' is a recoding of...
      factor(gender), # ... variable 'gender', coerced as factor
...
Now we have two variables with redundant information... let's get rid of ```gender```:

```{r recode_noGender}

att_filter_select_recode <- # over-write the data frame
  att_filter_select_recode %>%
...
This experiment has two independent variables, **drink** (beer or water) and **imagery** (neutral or negative). In our analysis, we wish to know the *separate* contribution of these two independent variables and their *interaction*. So, we need to separate ```condition``` into 2 variables:
...
# (i.e., values in 'participant', 'drink', and 'imagery' will be character) 
write_csv(
  att_filter_select_recode_long_sep_onego,
  here("tidy-data", "data_attitude.csv")
)
...
all_equal(att_filter_select_recode_long_sep,
          att_filter_select_recode_long_sep2)
