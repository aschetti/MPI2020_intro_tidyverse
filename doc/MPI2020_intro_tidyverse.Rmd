...
Recreate a plot similar to #2 using the [```yarrr```](https://cran.r-project.org/web/packages/yarrr/vignettes/pirateplot.html) package (**warning**: saving the plot to file can be tricky `r emo::ji("wink")`)
...
att_bootES <-
  ...
  mutate( # create new variable with bootstrapping results
    ...
    tidy = map(bootstrap, broom::tidy, conf.int = TRUE) # extract bootstrap results from each list into new columns
  ) %>%
  ungroup() # ungroup
